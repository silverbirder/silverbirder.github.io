...
const usage = () => {
  ...
  console.log(
    [
      "Usage: node ./scripts/migrate/fix-md026-no-trailing-punctuation.mjs [--src <dir>] [--dest <dir>] [--file <path>] [--files <paths>] [--overwrite] [--dry-run]",
      "",
      "Defaults:",
      "  --src  ./posts",
      "  --dest ./posts",
...
const stripTrailingPunctuation = (line) => {
  const match = line.match(/^(#{1,6}\s+)(.*)$/);
  if (!match) return line;
  const text = match[2].replace(/[。．.!！?？]+$/u, "").trimEnd();
  if (text === match[2]) return line;
  return `${match[1]}${text}`;
};
...
const transformContent = (content) => {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    const nextLine = stripTrailingPunctuation(line);
    if (nextLine !== line) {
      lines[i] = nextLine;
      changed = true;
    }
