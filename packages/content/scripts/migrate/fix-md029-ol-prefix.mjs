...
const usage = () => {
  ...
  console.log(
    [
      "Usage: node ./scripts/migrate/fix-md029-ol-prefix.mjs [--src <dir>] [--dest <dir>] [--file <path>] [--files <paths>] [--overwrite] [--dry-run]",
      "",
      "Defaults:",
      "  --src  ./posts",
      "  --dest ./posts",
...
const normalizeOrderedPrefix = (line) => {
  const orderedMatch = line.replace(/^(\s*(?:>\s*)*)(\d+)\.(\s+)/, "$11.$3");
  if (orderedMatch !== line) return orderedMatch;
  return line.replace(/^(\s*)-\s+(\d+)\.(\s+)/, "$11.$3");
};
...
const transformContent = (content) => {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    const nextLine = normalizeOrderedPrefix(line);
    if (nextLine !== line) {
      lines[i] = nextLine;
      changed = true;
    }
