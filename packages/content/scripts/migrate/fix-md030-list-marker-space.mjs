...
const usage = () => {
  ...
  console.log(
    [
      "Usage: node ./scripts/migrate/fix-md030-list-marker-space.mjs [--src <dir>] [--dest <dir>] [--file <path>] [--files <paths>] [--overwrite] [--dry-run]",
      "",
      "Defaults:",
      "  --src  ./posts",
      "  --dest ./posts",
...
const normalizeMarkerSpacing = (line) =>
  line.replace(/^(\s*(?:[-*+]|\d+\.))\s{2,}(\S)/, "$1 $2");
...
const transformContent = (content) => {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    const nextLine = normalizeMarkerSpacing(lines[i]);
    if (nextLine !== lines[i]) {
      lines[i] = nextLine;
      changed = true;
    }
