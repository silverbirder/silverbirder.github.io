---
title: 'はじめての、StorybookAddon開発体験記'
publishedAt: '2025-08-20'
summary: 'こんにちは。この度ちょっとした動機で、Storybookのアドオンをはじめて開発しました。本記事では、そのStorybookのアドオン開発の体験を共有したいと思います。'
tags: ["Storybook"]
---

こんにちは。
この前、ちょっとしたきっかけでStorybookのアドオンをはじめて開発しました。
...
このアドオンは、**端的にいうと Storybookの文字列・数値・配列の引数を、スライダーで調整できるアドオン** です。
作ろうと思った動機は、StorybookのArgTypesでrangeの値を使って、**データ増減に伴うデザイン崩れをチェック** していたのですが、
...
これは、Storybookのアドオン開発に必要なものが揃っていて、最初のとっかかりにはちょうどよかったです。

## アドオンの種類

アドオンキットを読んでいると、パネル、ツール、タブという3つの種類のアドオン開発があることを知りました。
公式ページでは、以下がわかりやすいです。
...
emotionを使ったことがある人なら、すぐに理解できるコードかと思います。
...
解決策が正しいのか分かりませんが、プレビューのデコレータからパラメータに関数がまだ残っているため、そのデータをパネルへ渡すようにしました。

以下のように、プレビューのデコレータでパラメータを設定し、パネルではそのパラメータを取得するようにしました。
