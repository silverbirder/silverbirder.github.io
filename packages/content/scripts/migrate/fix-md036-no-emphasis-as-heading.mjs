...
const usage = () => {
  ...
  console.log(
    [
      "Usage: node ./scripts/migrate/fix-md036-no-emphasis-as-heading.mjs [--src <dir>] [--dest <dir>] [--file <path>] [--files <paths>] [--overwrite] [--dry-run]",
      "",
      "Defaults:",
      "  --src  ./posts",
      "  --dest ./posts",
...
const convertEmphasisHeading = (line) => {
  ...
  const match = trimmed.match(/^(\*\*|__)(.+)\1$/) || trimmed.match(/^(\*|_)(.+)\1$/);
...
const transformContent = (content) => {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    const nextLine = convertEmphasisHeading(line);
    if (nextLine !== line) {
      lines[i] = nextLine;
      changed = true;
    }
