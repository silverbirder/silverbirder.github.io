...
const usage = () => {
  ...
  console.log(
    [
      "Usage: node ./scripts/migrate/fix-md001-heading-increment.mjs [--src <dir>] [--dest <dir>] [--file <path>] [--files <paths>] [--overwrite] [--dry-run]",
      "",
      "Defaults:",
      "  --src  ./posts",
      "  --dest ./posts",
...
const transformContent = (content) => {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (previousHeading > 0 && level > previousHeading + 1) {
      const nextLevel = previousHeading + 1;
      lines[i] = `${"#".repeat(nextLevel)} ${match[2]}`;
      changed = true;
      previousHeading = nextLevel;
      continue;
    }
