...
- 例では TypeScript と Vitest を取り上げますが、考え方は他の技術にもそのまま活かせます。
...
テストを書く狙いは次の3つに集約できます。

- 手動確認を減らして開発効率を高める
- バグの再混入を防いで品質を守る
- 振る舞いを言語化して仕様をはっきりさせる

背景や具体例は、拙稿「[Webフロントエンドにおける網羅的テストパターンガイド - zenn](https://zenn.dev/silverbirder/articles/c3de04c9e6dd58)」にまとめました。
...
ここからは、EC カートの合計金額を求める関数 `calculateCartTotal` を題材にします。
...
テストで参照する値（ここでは消費税率）が決まっているなら、定数として切り出しておきます。
...
テストは概ね「Arrange→Act→Assert」の順で進みます。
...
// より具体的な書き方
it('should return 14000 when items are ...', () => {
  // ...
})
```
