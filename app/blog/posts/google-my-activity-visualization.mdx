...
[Google Takeout](https://takeout.google.com/settings/takeout) からエクスポートしたマイアクティビティの ZIPファイル を読み込むと、ブラウザ内で以下の可視化を眺められます。
...
SQL をファイルにしておくとフォーマッタをかけやすく、差分レビューもしやすいので気に入っています。
...
1. `registerFileText` は、ローカルファイルシステムにインポートします。
...
ブラウザ上の DuckDB に対して SQL をそのまま流し込める体験は、新鮮さと懐かしさが同居します。
