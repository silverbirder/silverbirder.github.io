...
const usage = () => {
  ...
  console.log(
    [
      "Usage: node ./scripts/migrate/fix-md024-no-duplicate-heading.mjs [--src <dir>] [--dest <dir>] [--file <path>] [--files <paths>] [--overwrite] [--dry-run]",
      "",
      "Defaults:",
      "  --src  ./posts",
      "  --dest ./posts",
...
const transformContent = (content) => {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    const current = counts.get(text) ?? 0;
    counts.set(text, current + 1);

    if (current > 0) {
      const suffix = ` (${current + 1})`;
      lines[i] = `${match[1]} ${text}${suffix}`;
      changed = true;
    }
  }
