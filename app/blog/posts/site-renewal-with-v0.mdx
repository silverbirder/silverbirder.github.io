...
フォントやスペーシング、ラインなど細部にこだわるようになり、デザインの重要性を強く感じるようになりました。
...
UIデザインには、Reactコンポーネント、Tailwind、shadcn/uiの3つが用意されており、簡単にサンプルを作成してくれます。

特に衝撃的だったのは、わずか2回のやり取りでスイカゲームのようなWebアプリが作成できたことです。  
以下のリンクから確認できます。
...
罫線は縦のリズムが一定で、読み物に最適です。学生時代に使っていたノートのような親しみやすさがあり、付箋やシール、ページめくりなどの要素も追加しました。
...
ノートブック風デザインをさらに追求し、フォントについてもこだわるようにしました。  
...
ノートブックのページめくりを実現したかったのですが、うまく再現できませんでした。
